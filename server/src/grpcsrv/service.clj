...
(defn home-page
  [request]
  (ring-resp/response "Hello from demo-server, backed by Protojure Template!"))
...
(def write-chan (atom nil))
(def read-chan (atom nil))
...
(deftype Greeter []
  ...
  (SayRepeatHello
    ...
    (let [close-chan (:close-ch request)
          in-chan (:grpc-params request)
          out-chan (:grpc-out request)]
      (handle-client-disconnect close-chan)
      (log/info (format "->> subscribing [%s]" request))
      (reset! write-chan out-chan)
      (reset! read-chan in-chan)
      {:status 200
       :body out-chan})))
